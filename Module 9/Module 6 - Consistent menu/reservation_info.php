...
<head>
  ...
  <style id="critical-reservation-cards">
    ...
    /* Nudge the top CTA down slightly for better spacing */
    .hero .cta { margin-top: 14px !important; }
    /* Reduce overlap with hero by another 6px (was -36px) and tighten space below cards */
    .cards{max-width:1100px;margin:-30px auto 46px;display:flex;gap:32px;padding:0 24px;justify-content:center;z-index:5;position:relative}
    ...
    /* Larger, bolder slip-size labels and bigger anchor icons */
    .price-tile .label { font-size:18px; font-weight:800; }
    .price-icon svg { width:36px !important; height:36px !important; }
    ...
    .map-cta-card .btn.create{background:var(--gold);color:var(--navy);border-color:var(--gold);padding:14px 22px;border-radius:12px;box-shadow:0 10px 28px rgba(31,47,69,0.10);text-decoration:none;font-size:1.05rem}
    /* Page-local override: reduce hero overlap by 10px (from -50px to -40px) */
    .hero-registration + .cards { margin-top: -40px !important; }
  </style>
</head>
<body class="reservation-info">
  ...
  <?php
    // Use shared hero include so the hero matches site-wide styling
    $hero_title = 'Reserve Your Boat Slip';
    $hero_subtitle = '<p>Secure your spot at Moffat Bay Marina in the beautiful San Juan Islands.</p>';
  ...
  <!-- Login prompt and quick CTA moved from hero -->
  <div class="map-cta-card">
    <div class="cta-card">
      <div class="cta-text">You will be required to login or create an account to make a reservation.</div>
      <a class="btn create" href="slip_reservation.php">Make a Reservation</a>
    </div>
  </div>
