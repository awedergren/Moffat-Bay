...
if (isset($_POST['add_boat'])) {
  ...
  // JOIN WAITLIST (AJAX or normal POST)
  if (isset($_POST['join_waitlist'])) {
    // Minimal validation: require dates and slip size
    if (empty($startDate) || empty($endDate) || $slipSizeNum <= 0) {
      $error = "Please provide start date, end date, and slip size to join the waitlist.";
      ...
      try {
        // Compute waitlist position: count existing waitlist entries for same preferred_slip_size
        // that overlap the requested date range, then append.
        $countStmt = $pdo->prepare("SELECT COUNT(*) FROM waitlist WHERE preferred_slip_size = :size AND NOT (preferred_end_date < :start_date OR preferred_start_date > :end_date)");
        $countStmt->execute([':size' => (string)$slipSizeNum, ':start_date' => $startDate, ':end_date' => $endDate]);
        $pos = intval($countStmt->fetchColumn() ?? 0) + 1;

        // Determine boat ID for storage: if adding new, store NULL in boat_ID
        $storeBoatId = $boatID;
        if ($boatID === 'add_new') {
          $storeBoatId = null;
        }

        // Calculate months_duration roughly as days / 30 (at least 1)
        try {
          $startTs = strtotime($startDate);
          $endTs = strtotime($endDate);
          $monthsDuration = 1;
          if ($startTs !== false && $endTs !== false && $endTs > $startTs) {
            $monthsDuration = max(1, intval(round(($endTs - $startTs) / (60*60*24*30))));
          }
        } catch (Exception $e) {
          $monthsDuration = 1;
        }

        // Insert into waitlist table using actual column names
        $insSql = "INSERT INTO waitlist (user_ID, boat_ID, preferred_slip_size, preferred_start_date, preferred_end_date, months_duration, position_in_queue, date_created) VALUES (:user_id, :boat_id, :size, :start_date, :end_date, :months, :position, NOW())";
        $ins = $pdo->prepare($insSql);
        $ins->execute([
          ':user_id' => $userID,
          ':boat_id' => $storeBoatId,
          ':size' => (string)$slipSizeNum,
          ':start_date' => $startDate,
          ':end_date' => $endDate,
        ...
        // Recalculate positions for this slip size + overlapping date range so the queue
        // is always ordered by join time (date_created) with oldest first.
        try {
          $pdo->beginTransaction();
          // reset user variable and run an ordered update that assigns sequential positions
          $pdo->exec("SET @p := 0");
          $recalcSql = "
            UPDATE waitlist w
            JOIN (
              SELECT waitlist_ID, (@p := @p + 1) AS newpos FROM (
                SELECT waitlist_ID FROM waitlist
                WHERE preferred_slip_size = :size
                  AND NOT (preferred_end_date < :start_date OR preferred_start_date > :end_date)
                ORDER BY date_created ASC, waitlist_ID ASC
              ) AS sel
            ) AS seq ON w.waitlist_ID = seq.waitlist_ID
            SET w.position_in_queue = seq.newpos
          ";
          $recalcStmt = $pdo->prepare($recalcSql);
          $recalcStmt->execute([':size' => (string)$slipSizeNum, ':start_date' => $startDate, ':end_date' => $endDate]);

          // fetch the actual assigned position for the newly inserted row
          $posStmt = $pdo->prepare("SELECT position_in_queue FROM waitlist WHERE waitlist_ID = :id LIMIT 1");
          $posStmt->execute([':id' => $waitlistId]);
          $pos = intval($posStmt->fetchColumn() ?? $pos);

          $pdo->commit();
        } catch (Exception $e) {
          try { $pdo->rollBack(); } catch (Exception $__e) {}
          // If recalculation fails, keep previously computed $pos as fallback and continue
          log_db_error('Waitlist position recalculation failed: ' . $e->getMessage());
        }

        // Prepare JSON response for AJAX clients
        $resp = [
          'success' => true,
          'waitlist_id' => $waitlistId,
          'position' => $pos,
          'boat_id' => $storeBoatId,
          'slip_size' => (string)$slipSizeNum,
          'preferred_slip_size' => (string)$slipSizeNum,
          'start_date' => $startDate,
          'end_date' => $endDate,
          'preferred_start_date' => $startDate,
          'preferred_end_date' => $endDate,
          'months_duration' => intval($monthsDuration)
        ];
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
          header('Content-Type: application/json; charset=utf-8');
          ...
          // For non-AJAX POST, persist waitlist info to session and continue rendering
          $_SESSION['last_waitlist'] = $resp;
        }
      } catch (Exception $e) {
        log_db_error('Waitlist insert failed: ' . $e->getMessage());
        $error = "Unable to join the waitlist at this time.";
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
          header('Content-Type: application/json; charset=utf-8');
          echo json_encode(['error' => $error]);
          exit();
...
<style id="waitlist-visibility-override">
  /* Only show waitlist controls when the reservation card has the show-waitlist class */
    /* Increase vertical spacing above the availability message so the "No slips available" text
      sits lower — user requested a +20px increase originally; now reduce that by 10px. */
    .reservation-card #clientAvailabilityMsg { margin-top: 10px !important; }
  .reservation-card.show-waitlist #waitlistArea,
  .reservation-card.show-waitlist #joinWaitlistBtnPersistent,
  .reservation-card.show-waitlist #joinWaitlistBtnFallback,
  .reservation-card.show-waitlist #joinWaitlistBtn {
    display: block !important;
    z-index: 12000 !important;
    position: relative !important;
    pointer-events: auto !important;
  }
  .reservation-card.show-waitlist #waitlistArea { margin-top: -2px !important; text-align: center !important; }
  .reservation-card.show-waitlist #joinWaitlistBtnPersistent,
  .reservation-card.show-waitlist #joinWaitlistBtnFallback,
  .reservation-card.show-waitlist #joinWaitlistBtn,
  .reservation-card.show-waitlist .join-waitlist-btn {
    display: inline-block !important;
    ...
    <form id="reservationForm" method="POST" class="reservation-form">
      ...
      <div class="form-group reservation-cost">
        ...
        <div class="reservation-card-actions-inline" style="display:flex;flex-direction:column;justify-content:center;align-items:center;margin-top:82px;margin-bottom:15px;gap:0;">
          ...
          <!-- Persistent waitlist area (hidden by default). JS will toggle display when no slips available. -->
          <div id="waitlistArea" style="display:none;text-align:center;margin-top:-2px;">
            <button id="joinWaitlistBtnPersistent" type="button" class="btn-secondary" style="display:inline-block;background:#3F87A6;color:#fff;border:none;padding:12px 25px;border-radius:8px;font-size:15px;line-height:1.1;">Join the Waitlist</button>
          </div>
      </div>
      </div>
...
;(function(){
  ...
  // restore flow tracker label if we changed it for waitlist mode
  try{
    const steps = document.querySelectorAll('.reservation-steps .step');
    if(steps && steps.length >= 3){
      const step3 = steps[2];
      const lbl = step3.querySelector('.step-label');
      if(lbl && lbl.dataset && lbl.dataset._orig){ lbl.textContent = lbl.dataset._orig; delete lbl.dataset._orig; }
      // also clear any inline circle styling we may have set for waitlist mode
      try{ const circle = step3.querySelector('.step-circle'); if(circle){ circle.style.background = ''; circle.style.color = ''; circle.style.border = ''; circle.style.boxShadow = ''; } }catch(e){}
      step3.classList.remove('waitlist-mode');
    }
  }catch(e){}
...
(function(){
    ...
    try{
      if(!document.querySelector('style[data-sr-style]')){
        const css = '\n.reservation-review .dates{display:flex;justify-content:center;width:100% !important;}\n.reservation-review .dates .value{text-align:center !important;}\n.reservation-review h3{ text-align:center !important; margin-left:auto !important; margin-right:auto !important; }\n.reservation-review .panel-inner{ /* ensure predictable layout inside review */ display:grid; grid-template-columns:repeat(3,1fr); gap:24px; align-items:start; }\n.reservation-review .confirm-wrapper{display:flex;justify-content:center;gap:12px;margin-top:12px;width:100% !important;align-items:center; grid-column:1 / -1;}\n.reservation-review .confirm-wrapper button{margin:0 !important;}\n';
          ...
          (document.head || document.documentElement).appendChild(s);
          // Ensure coral-outline buttons invert to filled coral on hover (consistent across pages)
          if(!document.querySelector('style[data-coral-style]')){
            const coralCss = '\n.btn-cancel, .reservation-actions .cancel-reservation, .reservation-actions .btn.cancel { background: transparent !important; color: #E8896B !important; border: 1px solid #E8896B !important; }\n.btn-cancel:hover, .reservation-actions .cancel-reservation:hover, .reservation-actions .btn.cancel:hover { background: #E8896B !important; color: #fff !important; border-color: #E8896B !important; }\n#joinWaitlistBtnPersistent { background: transparent; color: #E8896B; border: 1px solid #E8896B; padding: 8px 12px; border-radius:8px; }\n#joinWaitlistBtnPersistent:hover { background: #E8896B; color: #fff; border-color: #E8896B; }\n';
            const sc = document.createElement('style'); sc.setAttribute('data-coral-style','1'); sc.appendChild(document.createTextNode(coralCss));
  ...
  availBtn.addEventListener('click', function(e){
    ...
    fetch(window.location.href, {method: 'POST', body: fd, headers: {'X-Requested-With':'XMLHttpRequest'}})
      ...
      .then(json => {
        ...
        msg.innerHTML = '';
        if(json.error){
          // If server returned an error but also provided an availability count,
          // continue so we can show the waitlist when availableCount === 0.
          if (typeof json.availableCount === 'undefined' || json.availableCount !== 0) {
            msg.className = 'alert alert-error'; msg.textContent = json.error; return;
          }
          // otherwise fallthrough and handle availableCount === 0 below
          msg.className = 'alert alert-error'; msg.textContent = json.error;
        }
        const cnt = json.availableCount || 0;
        // Set inline styles on the persistent waitlist area/button so visibility is
        // guaranteed regardless of other styles. Use !important via setProperty.
        try{
          var wa = document.getElementById('waitlistArea');
          var pbtn = document.getElementById('joinWaitlistBtnPersistent');
          if(cnt === 0){
            console.debug('Availability: 0 slips — showing waitlist area');
            if(wa){ try{ wa.style.setProperty('display','block','important'); }catch(e){ wa.style.display='block'; } }
            if(pbtn){ try{ pbtn.style.setProperty('display','inline-block','important'); }catch(e){ pbtn.style.display='inline-block'; } }
          } else {
            console.debug('Availability: >0 slips — hiding waitlist area');
            if(wa){ try{ wa.style.setProperty('display','none','important'); }catch(e){ wa.style.display='none'; } }
            if(pbtn){ try{ pbtn.style.setProperty('display','none','important'); }catch(e){ pbtn.style.display='none'; } }
          }
        }catch(e){ console.debug('Waitlist UI toggle error', e); }
        if(cnt > 0){
          ...
          // remove waitlist class when slips are available
          try{ var c = document.querySelector('.reservation-card'); if(c) c.classList.remove('show-waitlist'); }catch(e){}
          ...
          msg.innerHTML = 'No slips available for those dates.';
          // Keep the user on Step 2 (Select Slip) and offer a client-side Join Waitlist button
          try{ setFlowStep(2); }catch(e){}
          // ensure persistent waitlist class is set so CSS shows the persistent button
          try{ var c = document.querySelector('.reservation-card'); if(c) c.classList.add('show-waitlist'); }catch(e){}

          // Show only a single waitlist control. Remove any previous inline/fallback
          // buttons and prefer the persistent `#joinWaitlistBtnPersistent` element
          // that already exists in the form. If the persistent element is missing,
          // create one in the availability message (but only one).
          try{
            // remove old inline/fallback buttons (avoid duplicates)
            document.querySelectorAll('#joinWaitlistBtn, #joinWaitlistBtnFallback').forEach(function(n){ try{ n.remove(); }catch(e){} });

            var pbtn = document.getElementById('joinWaitlistBtnPersistent');
            var wa = document.getElementById('waitlistArea');
            if(pbtn){
              // ensure persistent area visible
              try{ wa && wa.style.setProperty('display','block','important'); }catch(e){ if(wa) wa.style.display = 'block'; }
              try{ pbtn.style.setProperty('display','inline-block','important'); }catch(e){ pbtn.style.display = 'inline-block'; }
              // move the persistent area next to the inline actions (if present)
              try{
                var inlineActions = form.querySelector('.reservation-card-actions-inline');
                if(inlineActions && wa && inlineActions.parentNode && wa.parentNode !== inlineActions.parentNode){
                  inlineActions.parentNode.insertBefore(wa, inlineActions.nextSibling);
                }
              }catch(e){}
            } else {
              // create a single, classed button inside the message only if none exists
              if(!document.querySelector('.join-waitlist-btn')){
                var btn = document.createElement('button');
                btn.type = 'button'; btn.className = 'join-waitlist-btn btn-secondary'; btn.textContent = 'Join the Waitlist';
                btn.style.background = '#3F87A6'; btn.style.color = '#fff'; btn.style.border = 'none';
                btn.style.padding = '12px 25px'; btn.style.borderRadius = '8px'; btn.style.display = 'inline-block'; btn.style.margin = '8px auto';
                var wrap = document.createElement('div'); wrap.style.textAlign = 'center'; wrap.style.marginTop = '18px'; wrap.appendChild(btn);
                msg.appendChild(wrap);
              }
            }
          }catch(e){}

          // remove any previous slip select and hide confirm button
          const wrapper = document.getElementById('availableSlipWrapper'); if(wrapper) wrapper.remove();
          const confirmBtn = document.getElementById('confirmBtn'); if(confirmBtn) confirmBtn.style.display = 'none';
        }
  ...
  // Previously there was a prompt-based handler here; replaced by the
  // AJAX-based Join Waitlist handler added near the end of the file so
  // we avoid blocking native dialogs and duplicate behavior.
...
?>
<script>
// Client-side: handle Join the Waitlist clicks via AJAX and render a step-3 review
// Non-blocking error display for waitlist actions
function showWaitlistError(msg){
  try{
    var form = document.getElementById('reservationForm');
    var existing = document.getElementById('waitlistError');
    if(!existing){
      existing = document.createElement('div');
      existing.id = 'waitlistError';
      existing.className = 'form-error';
      existing.style.margin = '8px 0';
      existing.style.padding = '10px';
      existing.style.background = '#fff3f3';
      existing.style.border = '1px solid #ffb3b3';
      existing.style.color = '#8a1f1f';
      existing.style.borderRadius = '6px';
      if(form){
        var insertBeforeNode = form.querySelector('.form-actions') || form.firstChild;
        form.insertBefore(existing, insertBeforeNode);
      } else {
        document.body.insertBefore(existing, document.body.firstChild);
      }
    }
    existing.textContent = msg || 'An error occurred.';
    existing.style.display = 'block';
    // auto-hide after 8s
    setTimeout(function(){ try{ existing.style.display = 'none'; }catch(e){} }, 8000);
  }catch(e){ try{ console.error(msg); }catch(e){} }
}

// Hide/show reservation input fields and availability controls while review is visible
function hideReservationInputs(){
  try{
    var groups = ['.form-group.start-date', '.form-group.end-date', '.form-group.select-boat', '.form-group.slip-size', '#addBoatPanel', '.form-grid', '.reservation-cost', '.cost-summary'];
    groups.forEach(function(sel){
      try{ var el = document.querySelector(sel); if(el){ el.dataset._prevDisplay = el.style.display || ''; el.style.display = 'none'; } }catch(e){}
      try{ var els = document.querySelectorAll(sel); if(els && els.length>1){ els.forEach(function(x){ x.dataset._prevDisplay = x.style.display || ''; x.style.display = 'none'; }); } }catch(e){}
    });
    // also hide the availability button if present
    try{ var btn = document.querySelector("button[name='check_availability']"); if(btn){ btn.dataset._prevDisplay = btn.style.display || ''; btn.style.display = 'none'; } }catch(e){}
    // hide persistent waitlist UI if present
    try{ var wa = document.getElementById('waitlistArea'); if(wa){ wa.dataset._prevDisplay = wa.style.display || ''; wa.style.display = 'none'; } }catch(e){}
    try{ var jw = document.getElementById('joinWaitlistBtnPersistent'); if(jw){ jw.dataset._prevDisplay = jw.style.display || ''; jw.style.display = 'none'; } }catch(e){}
    try{ var joins = document.querySelectorAll('.join-waitlist-btn'); if(joins){ joins.forEach(function(x){ x.dataset._prevDisplay = x.style.display || ''; x.style.display = 'none'; }); } }catch(e){}
  }catch(e){}
}

function showReservationInputs(){
  try{
    var groups = ['.form-group.start-date', '.form-group.end-date', '.form-group.select-boat', '.form-group.slip-size', '#addBoatPanel', '.form-grid', '.reservation-cost', '.cost-summary'];
    groups.forEach(function(sel){
      try{ var el = document.querySelector(sel); if(el){ el.style.display = (el.dataset && el.dataset._prevDisplay) ? el.dataset._prevDisplay : ''; delete el.dataset._prevDisplay; } }catch(e){}
      try{ var els = document.querySelectorAll(sel); if(els && els.length>1){ els.forEach(function(x){ x.style.display = (x.dataset && x.dataset._prevDisplay) ? x.dataset._prevDisplay : ''; delete x.dataset._prevDisplay; }); } }catch(e){}
    });
    try{ var btn = document.querySelector("button[name='check_availability']"); if(btn){ btn.style.display = (btn.dataset && btn.dataset._prevDisplay) ? btn.dataset._prevDisplay : ''; delete btn.dataset._prevDisplay; } }catch(e){}
    // restore persistent waitlist UI
    try{ var wa = document.getElementById('waitlistArea'); if(wa){ wa.style.display = (wa.dataset && wa.dataset._prevDisplay) ? wa.dataset._prevDisplay : ''; delete wa.dataset._prevDisplay; } }catch(e){}
    try{ var jw = document.getElementById('joinWaitlistBtnPersistent'); if(jw){ jw.style.display = (jw.dataset && jw.dataset._prevDisplay) ? jw.dataset._prevDisplay : ''; delete jw.dataset._prevDisplay; } }catch(e){}
    try{ var joins = document.querySelectorAll('.join-waitlist-btn'); if(joins){ joins.forEach(function(x){ x.style.display = (x.dataset && x.dataset._prevDisplay) ? x.dataset._prevDisplay : ''; delete x.dataset._prevDisplay; }); } }catch(e){}
  }catch(e){}
}

// Format YYYY-MM-DD into "Mon D, YYYY" (e.g., "Mar 3, 2026")
function formatPreferredDate(dstr){
  try{
    if(!dstr) return '';
    ...
    var parts = (dstr||'').split('-'); if(parts.length<3) return dstr;
    var months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    var y = parts[0], m = parseInt(parts[1],10), day = parseInt(parts[2],10);
    return (months[(m-1 + 12) % 12] || parts[1]) + ' ' + day + ', ' + y;
  }catch(e){ return dstr; }
}
...
document.addEventListener('click', function(ev){
  try{
    ...
    var isJoin = (t.id === 'joinWaitlistBtnPersistent' || t.id === 'joinWaitlistBtn' || t.id === 'joinWaitlistBtnFallback' || (t.classList && t.classList.contains('join-waitlist-btn')));
    if(!isJoin) return;
    try{ ev.preventDefault(); }catch(e){}
    var form = document.getElementById('reservationForm'); if(!form) return;
    var fd = new FormData(form); fd.set('join_waitlist','1');
    fetch(window.location.href, { method: 'POST', body: fd, headers: {'X-Requested-With':'XMLHttpRequest'} })
      .then(function(r){ return r.json(); })
      .then(function(json){
        if(!json || json.error){ try{ showWaitlistError(json && json.error ? json.error : 'Unable to join the waitlist at this time.'); }catch(e){} return; }
        try{ setFlowStep(3);
          // Mark the visual flow tracker for waitlist mode (only for this path)
          try{
            const steps = document.querySelectorAll('.reservation-steps .step');
            if(steps && steps.length >= 3){
              const step3 = steps[2];
              const lbl = step3.querySelector('.step-label');
              if(lbl){ lbl.dataset._orig = lbl.textContent || ''; lbl.textContent = 'Waitlist'; }
              // style the circle as active (gold) for waitlist display
              const circle = step3.querySelector('.step-circle');
              if(circle){ circle.style.background = '#F4C26B'; circle.style.color = '#1F2F45'; circle.style.border = '2px solid rgba(31,47,69,0.06)'; circle.style.boxShadow = '0 8px 22px rgba(31,47,69,0.12)'; }
              step3.classList.add('waitlist-mode');
            }
          }catch(e){}
        }catch(e){}
        try{ var existing = document.getElementById('waitlistReviewPanel'); if(existing) existing.remove(); }catch(e){}
        var review = document.createElement('div'); review.id = 'waitlistReviewPanel'; review.className = 'reservation-review'; review.style.width = '100%'; review.style.boxSizing = 'border-box';
        var panel = document.createElement('div'); panel.className = 'panel-inner'; panel.style.maxWidth = '980px'; panel.style.margin = '0 auto'; panel.style.padding = '18px';
        var hdr = document.createElement('h3'); hdr.textContent = 'Waitlist Confirmation'; hdr.style.textAlign = 'center'; hdr.style.marginTop='6px'; hdr.style.marginBottom='14px'; hdr.style.fontSize='24px'; hdr.style.fontWeight='800'; hdr.style.gridColumn = '1 / -1'; panel.appendChild(hdr);
        var userFull = (document.getElementById('currentUserFull')||{}).value || ''; var email = (document.getElementById('currentUserEmail')||{}).value || ''; var phone = (document.getElementById('currentUserPhone')||{}).value || '';
        var boatLabel = json.boat_name || ((form.querySelector('select[name="boat_id"] option:checked')||{}).textContent || '');
        var slipSize = json.slip_size || ((form.querySelector('select[name="slip_size"]')||{}).value || '');
        var start = json.start_date || ((form.querySelector('input[name="start_date"]')||{}).value || ''); var end = json.end_date || ((form.querySelector('input[name="end_date"]')||{}).value || '');
        var pos = json.position || '';
        // Build a clean 4-column layout using flex so values distribute evenly with anchors
        var infoGrid = document.createElement('div');
        infoGrid.style.display = 'flex';
        infoGrid.style.justifyContent = 'center';
        infoGrid.style.alignItems = 'flex-start';
        infoGrid.style.marginBottom = '8px';
        infoGrid.style.width = '100%';
        infoGrid.style.boxSizing = 'border-box';
        infoGrid.style.padding = '0 24px';

        function makeCol(label, valueHtml){
          var col = document.createElement('div');
          col.innerHTML = '<div class="label" style="font-weight:700;margin-bottom:6px">'+label+'</div><div class="value">'+(valueHtml||'')+'</div>';
          col.style.boxSizing = 'border-box';
          col.style.flex = '0 1 180px';
          col.style.minWidth = '140px';
          col.style.margin = '0 22px';
          col.style.textAlign = 'center';
          return col;
        }

        // left anchor (visual margin)
        var leftAnchor = document.createElement('div'); leftAnchor.style.flex = '0 0 6%'; infoGrid.appendChild(leftAnchor);
        // content columns
        infoGrid.appendChild(makeCol('User', (userFull || email || '')));
        infoGrid.appendChild(makeCol('Boat', (boatLabel || 'n/a')));
        infoGrid.appendChild(makeCol('Slip Size', slipSize ? (slipSize + ' ft') : ''));
        infoGrid.appendChild(makeCol('Waitlist Position', '<span style="font-weight:800;font-size:18px">#' + (pos || '') + '</span>'));
        // right anchor (visual margin)
        var rightAnchor = document.createElement('div'); rightAnchor.style.flex = '0 0 6%'; infoGrid.appendChild(rightAnchor);

        panel.appendChild(infoGrid);

        // Dates span the full width and are centered
        var dates = document.createElement('div');
        dates.style.gridColumn = '1 / -1';
        dates.style.textAlign = 'center';
        dates.style.marginTop = '6px';
        dates.innerHTML = '<div class="label">Dates</div><div class="value">' + formatPreferredDate(start) + ' — ' + formatPreferredDate(end) + '</div>';
        panel.appendChild(dates);

        var cwrap = document.createElement('div'); cwrap.className = 'confirm-wrapper'; cwrap.style.display='flex'; cwrap.style.justifyContent='center'; cwrap.style.gap='12px'; cwrap.style.marginTop='18px';
        var done = document.createElement('button'); done.type='button'; done.className='btn-secondary'; done.textContent='Start a new Reservation'; done.style.display='inline-block'; done.style.background='#3F87A6'; done.style.color='#fff'; done.style.border='none'; done.style.padding='12px 25px'; done.style.borderRadius='8px'; done.addEventListener('click', function(){ try{ location.replace(window.location.pathname + '?reset_reservation=1'); }catch(e){ location.reload(); } });
        var close = document.createElement('button'); close.type='button'; close.className='btn-myaccount'; close.textContent='My Account'; close.style.background='#F4C26B'; close.style.color='#000'; close.style.border='none'; close.style.padding='12px 25px'; close.style.borderRadius='8px'; close.style.fontWeight='700'; close.addEventListener('click', function(){ try{ location.replace('MyAccount.php'); }catch(e){ location.reload(); } });
        cwrap.appendChild(done); cwrap.appendChild(close); panel.appendChild(cwrap); review.appendChild(panel);
        try{ var actions = form.querySelector('.form-actions') || form; actions.parentNode.insertBefore(review, actions); }catch(e){}
        try{ hideReservationInputs();
          // additionally hide stray labels matching specific text if still visible
          try{
            var labels = document.querySelectorAll('.form-group label');
            labels.forEach(function(lbl){
              try{
                var txt = (lbl.textContent||'').trim();
                if(['Start Date','End Date','Select Boat','Slip Size'].indexOf(txt) !== -1){
                  lbl.dataset._prevDisplay = lbl.style.display || '';
        ...
        // Ensure the flow tracker label is updated for the waitlist view and remains set.
        try{
          function applyWaitlistLabel(){
            try{
              const steps = document.querySelectorAll('.reservation-steps .step');
              if(steps && steps.length >= 3){
                const step3 = steps[2];
                const lbl = step3.querySelector('.step-label');
                if(lbl){ lbl.dataset._orig = lbl.dataset._orig || (lbl.textContent || ''); lbl.textContent = 'Waitlist'; }
                const circle = step3.querySelector('.step-circle');
                if(circle){ circle.style.background = '#F4C26B'; circle.style.color = '#1F2F45'; circle.style.border = '2px solid rgba(31,47,69,0.06)'; circle.style.boxShadow = '0 8px 22px rgba(31,47,69,0.12)'; }
                step3.classList.add('waitlist-mode');
                const card = document.querySelector('.reservation-card'); if(card) card.classList.add('flow-waitlist');
              }
            }catch(e){}
          }
          applyWaitlistLabel();
          setTimeout(applyWaitlistLabel, 60);
        }catch(e){}
      }).catch(function(err){ try{ showWaitlistError('Unable to join the waitlist at this time.'); }catch(e){} });
  }catch(e){}
}, false);
</script>
<?php
