...
if ($isAjaxRequest) {
    // Append a short debug record (headers, session, POST) to a local log file for troubleshooting.
        try {
            // intentionally left blank for AJAX handling when debugging is disabled
...
$availableSlipSizes = [];
$availableDocks = [];
// detect current employee position (if logged-in employee) so UI can show manager-only options
$employeePosition = '';
try {
    if ($pdo) {
        ...
        // fetch employee position for current session employee (non-fatal)
        if (!empty($_SESSION['employee_id'])) {
            try {
                $es = $pdo->prepare('SELECT position FROM Employees WHERE employee_ID = ? LIMIT 1');
                $es->execute([intval($_SESSION['employee_id'])]);
                $employeePosition = trim((string)($es->fetchColumn() ?? ''));
            } catch (Exception $_e) { /* ignore */ }
        }
        // detect Employees phone column name if present (to support schema variants)
        $employeePhoneCol = null;
        try {
            $colStmt = $pdo->prepare("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'Employees'");
            $colStmt->execute();
            $empCols = $colStmt->fetchAll(PDO::FETCH_COLUMN);
            $phoneCandidates = ['phone','phone_number','employee_phone','mobile','phone_mobile','telephone'];
            foreach ($phoneCandidates as $pc) { if (in_array($pc, $empCols)) { $employeePhoneCol = $pc; break; } }
        } catch (Exception $_e) { $employeePhoneCol = null; }
    }
} catch (Exception $ex) {
...
$statusMsg = '';
if (!empty($_SESSION['statusMsg'])) { $statusMsg = $_SESSION['statusMsg']; unset($_SESSION['statusMsg']); }
$errors = [];
$searchResults = [];
...
// If the request used query params to ask to edit a user or boat (GET navigation), load those rows so inline forms can populate
try {
    if ($pdo) {
        if (empty($editUser) && !empty($_REQUEST['action']) && $_REQUEST['action'] === 'edit_user' && !empty($_REQUEST['user_id'])) {
            $uid = intval($_REQUEST['user_id']);
            $sq = $pdo->prepare('SELECT user_ID, email, first_name, last_name FROM users WHERE user_ID = ? LIMIT 1');
            $sq->execute([$uid]);
            $editUser = $sq->fetch(PDO::FETCH_ASSOC) ?: null;
        }
        if (empty($editEmployee) && !empty($_REQUEST['action']) && $_REQUEST['action'] === 'edit_employee' && !empty($_REQUEST['employee_id'])) {
            $eid = intval($_REQUEST['employee_id']);
            $se = $pdo->prepare('SELECT * FROM Employees WHERE employee_ID = ? LIMIT 1');
            $se->execute([$eid]);
            $editEmployee = $se->fetch(PDO::FETCH_ASSOC) ?: null;
        }
        if (empty($editBoat) && !empty($_REQUEST['action']) && $_REQUEST['action'] === 'edit_boat' && !empty($_REQUEST['boat_id'])) {
            $bid = intval($_REQUEST['boat_id']);
            $sb = $pdo->prepare('SELECT * FROM boats WHERE boat_ID = ? LIMIT 1'); $sb->execute([$bid]); $editBoat = $sb->fetch(PDO::FETCH_ASSOC) ?: null;
        }
    }
} catch (Exception $_) { /* ignore load errors; rendering will show no details */ }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $pdo) {
    ...
    try {
        ...
        if ($action === 'search') {
            ...
            // task-driven searches (normalize task value and match loosely)
            $isTaskSearch = false;
            $rawTask = strtolower(trim((string)($taskType ?? ($_POST['task_type'] ?? ''))));
            if ($rawTask !== '') {
                if (strpos($rawTask,'checkin') !== false || strpos($rawTask,'checkins') !== false) {
                    // reservations starting today or within next 24h, status confirmed or pending
                    $sql = "SELECT r.*, u.email, u.first_name, u.last_name, u.phone AS user_phone, b.boat_name, b.boat_length, s.slip_size, s.location_code, (SELECT COUNT(*) FROM employee_payments ep WHERE ep.reservation_ID = r.reservation_ID) AS payments_count, (SELECT ep.amount FROM employee_payments ep WHERE ep.reservation_ID = r.reservation_ID ORDER BY ep.date_created DESC LIMIT 1) AS last_payment_amount, (SELECT ep.card_last4 FROM employee_payments ep WHERE ep.reservation_ID = r.reservation_ID ORDER BY ep.date_created DESC LIMIT 1) AS last_payment_last4, (SELECT ep.method FROM employee_payments ep WHERE ep.reservation_ID = r.reservation_ID ORDER BY ep.date_created DESC LIMIT 1) AS last_payment_method FROM reservations r LEFT JOIN users u ON r.user_ID = u.user_ID LEFT JOIN boats b ON r.boat_ID = b.boat_ID LEFT JOIN slips s ON r.slip_ID = s.slip_ID WHERE ((DATE(r.start_date) = CURDATE()) OR (r.start_date >= NOW() AND r.start_date <= DATE_ADD(NOW(), INTERVAL 1 DAY))) AND (r.reservation_status = 'confirmed' OR r.reservation_status = 'pending') ORDER BY r.start_date ASC";
                    $stmt = $pdo->prepare($sql); $stmt->execute([]); $searchResults = $stmt->fetchAll(PDO::FETCH_ASSOC); $resultType = 'tasks_checkins'; $isTaskSearch = true;
                } elseif (strpos($rawTask,'payment') !== false || strpos($rawTask,'payments') !== false) {
                    ...
                    $stmt = $pdo->prepare($sql); $stmt->execute([]); $searchResults = $stmt->fetchAll(PDO::FETCH_ASSOC); $resultType = 'tasks_payments_due'; $isTaskSearch = true;
                } elseif (strpos($rawTask,'completion') !== false || strpos($rawTask,'completions') !== false) {
                    ...
                    $stmt = $pdo->prepare($sql); $stmt->execute([]); $searchResults = $stmt->fetchAll(PDO::FETCH_ASSOC); $resultType = 'tasks_completions'; $isTaskSearch = true;
                } elseif (strpos($rawTask,'waitlist') !== false) {
                    // waitlist deletions within 72 hours
                    ...
                    foreach ($candidates as $w) {
                        ...
                        $sc = $pdo->prepare('SELECT COUNT(*) FROM slips WHERE slip_size = ?'); $sc->execute([$size]); $totalSlips = intval($sc->fetchColumn() ?? 0);
                        ...
                        $rc->execute([$size, $start, $end]); $reserved = intval($rc->fetchColumn() ?? 0); $available = max(0, $totalSlips - $reserved);
                        if ($available === 0) { $searchResults[] = $w; }
                    }
                        $resultType = 'tasks_waitlist_delete'; $isTaskSearch = true;
                }
            }
            else {
                ...
                if (empty($isTaskSearch) && $scope === 'reservations') {
                $clauses = [];
                $params = [];
                if (!empty($_POST['date'])) {
            ...
            // Waitlist search
            elseif (empty($isTaskSearch) && $scope === 'waitlist') {
                $clauses = [];
                $params = [];
                if (!empty($_POST['date'])) {
            ...
            // Employees search (manager-only option)
            elseif (empty($isTaskSearch) && $scope === 'employees') {
                $clauses = [];
                $params = [];
                if (!empty($_POST['email'])) { $clauses[] = 'email = ?'; $params[] = $_POST['email']; }
                if (!empty($_POST['filter_owner'])) { $clauses[] = '(first_name LIKE ? OR last_name LIKE ?)'; $params[] = '%'.$_POST['filter_owner'].'%'; $params[] = '%'.$_POST['filter_owner'].'%'; }
                $where = count($clauses) ? 'WHERE ' . implode(' AND ', $clauses) : '';
                $selectCols = 'employee_ID, email, first_name, last_name, position, date_created';
                if (!empty($employeePhoneCol)) {
                    // include the detected phone column in the select so results contain phone values
                    $selectCols = 'employee_ID, email, ' . $employeePhoneCol . ', first_name, last_name, position, date_created';
                }
                $sql = "SELECT $selectCols FROM Employees $where LIMIT 500";
                $stmt = $pdo->prepare($sql);
                $stmt->execute($params);
                $searchResults = $stmt->fetchAll(PDO::FETCH_ASSOC);
            }
            // Users search
            elseif (empty($isTaskSearch) && $scope === 'users') {
                $clauses = [];
                $params = [];
                if (!empty($_POST['email'])) { $clauses[] = 'email = ?'; $params[] = $_POST['email']; }
            ...
            // Boats search
            elseif (empty($isTaskSearch) && $scope === 'boats') {
                $clauses = [];
                $params = [];
            ...
            // Payments search
            elseif (empty($isTaskSearch) && $scope === 'payments') {
                $clauses = [];
                $params = [];
        ...
        // Employee edit (manager-used)
        if ($action === 'edit_employee' && !empty($_POST['employee_id'])) {
            $id = intval($_POST['employee_id']);
            $s = $pdo->prepare('SELECT * FROM Employees WHERE employee_ID = ? LIMIT 1');
            $s->execute([$id]);
            $editEmployee = $s->fetch(PDO::FETCH_ASSOC) ?: null;
        }
        ...
        // Save employee changes (manager-only)
        if ($action === 'save_employee' && !empty($_POST['employee_id'])) {
            $id = intval($_POST['employee_id']);
            // include position and phone (use detected Employees phone column when available)
            $fields = ['email','first_name','last_name','position']; $sets=[]; $params=[];
            if (!empty($employeePhoneCol)) $fields[] = $employeePhoneCol; else $fields[] = 'phone';
            foreach ($fields as $f) { if (isset($_POST[$f])) { $sets[] = "$f = ?"; $params[] = $_POST[$f]; } }
            if ($sets) {
                // require employee password confirmation for edits
                $pwd = $_POST['confirm_password'] ?? '';
                    if (empty($pwd)) {
                    $errors[] = 'Employee password is required to save employee changes.';
                    $s = $pdo->prepare('SELECT * FROM Employees WHERE employee_ID = ? LIMIT 1'); $s->execute([$id]); $editEmployee = $s->fetch(PDO::FETCH_ASSOC) ?: null;
                } else {
                    $empId = intval($_SESSION['employee_id'] ?? 0);
                    $ep = $pdo->prepare('SELECT password FROM Employees WHERE employee_ID = ? LIMIT 1'); $ep->execute([$empId]); $hash = $ep->fetchColumn() ?: '';
                        if (!password_verify($pwd,$hash)) {
                        ...
                        $params[] = $id; $sql = 'UPDATE Employees SET '.implode(', ',$sets).' WHERE employee_ID = ?'; $u = $pdo->prepare($sql); $u->execute($params); $statusMsg = 'Employee updated.';
                    }
                }
            }
        }

        // Show add-employee form (manager-only) - triggered by action=add_employee via POST/GET
        if ($action === 'add_employee') {
            // nothing server-side to load; rendering will display the create form
        }

        // Create new employee (manager-only)
        if ($action === 'create_employee') {
            // ensure current user is a manager
            if (strtolower(trim($employeePosition)) !== 'manager') {
                $errors[] = 'Only managers may create employees.';
            } else {
                // require manager password confirmation
                $mgrPwd = $_POST['confirm_password'] ?? '';
                if (empty($mgrPwd)) {
                    $errors[] = 'Manager password confirmation is required.';
                } else {
                    $mgrId = intval($_SESSION['employee_id'] ?? 0);
                    $ep = $pdo->prepare('SELECT password FROM Employees WHERE employee_ID = ? LIMIT 1'); $ep->execute([$mgrId]); $mgrHash = $ep->fetchColumn() ?: '';
                    if (!password_verify($mgrPwd, $mgrHash)) {
                        $errors[] = 'Invalid manager password.';
                    }
                }
                ...
                if (empty($newEmail)) $errors[] = 'New employee email is required.';
                if (empty($newFirst)) $errors[] = 'New employee first name is required.';
                if (empty($newLast)) $errors[] = 'New employee last name is required.';
                if (empty($newPosition)) $errors[] = 'New employee position is required.';
                if (empty($newPass)) $errors[] = 'New employee password is required.';
                // if no errors, insert
                if (!$errors) {
                    try {
                        $cols = ['email','first_name','last_name','position'];
                        $placeholders = ['?','?','?','?'];
                        $params = [$newEmail,$newFirst,$newLast,$newPosition];
                        // phone column
                        $phoneCol = !empty($employeePhoneCol) ? $employeePhoneCol : 'phone';
                        $newPhone = $_POST[$phoneCol] ?? '';
                        if ($newPhone !== '') { $cols[] = $phoneCol; $placeholders[] = '?'; $params[] = $newPhone; }
                        // password
                        $pwdHash = password_hash($newPass, PASSWORD_DEFAULT);
                        $cols[] = 'password'; $placeholders[] = '?'; $params[] = $pwdHash;
                        $sql = 'INSERT INTO Employees ('.implode(',', $cols).') VALUES ('.implode(',', $placeholders).')';
                        $ins = $pdo->prepare($sql); $ins->execute($params);
                        $_SESSION['statusMsg'] = 'Employee created.';
                        $statusMsg = 'Employee created.';
                        if (!headers_sent()) {
                            header('Location: ' . basename(__FILE__));
                            exit;
                        } else {
                            echo '<script>window.location = "' . basename(__FILE__) . '";</script>';
                            exit;
                        }
                    } catch (Exception $ex) {
                        $errors[] = 'Error creating employee: ' . $ex->getMessage();
                    }
                }
            }
        }
        ...
        if ($action === 'remove_employee' && !empty($_POST['employee_id'])) {
            $eid = intval($_POST['employee_id']);
            if (strtolower(trim($employeePosition)) !== 'manager') {
                $errors[] = 'Only managers may remove employees.';
            } else {
                // prevent self-deletion
                if (!empty($_SESSION['employee_id']) && intval($_SESSION['employee_id']) === $eid) {
                    $errors[] = 'You cannot remove your own account.';
                } else {
                    $mgrPwd = $_POST['confirm_password'] ?? '';
                    if (empty($mgrPwd)) {
                        $errors[] = 'Manager password confirmation is required to remove an employee.';
                    } else {
                        $mgrId = intval($_SESSION['employee_id'] ?? 0);
                        $ep = $pdo->prepare('SELECT password FROM Employees WHERE employee_ID = ? LIMIT 1'); $ep->execute([$mgrId]); $mgrHash = $ep->fetchColumn() ?: '';
                        if (!password_verify($mgrPwd, $mgrHash)) {
                            ...
                            try {
                                $d = $pdo->prepare('DELETE FROM Employees WHERE employee_ID = ?'); $d->execute([$eid]);
                                $_SESSION['statusMsg'] = 'Employee removed.';
                                if (!headers_sent()) { header('Location: ' . basename(__FILE__)); exit; }
                                else { echo '<script>window.location = "' . basename(__FILE__) . '";</script>'; exit; }
                            } catch (Exception $ex) { $errors[] = 'Error removing employee: ' . $ex->getMessage(); }
                        }
                    }
                }
            }
        ...
        // Add reservation from waitlist entry
        if ($action === 'add_reservation_from_waitlist' && !empty($_POST['waitlist_id'])) {
            $wid = intval($_POST['waitlist_id']);
            $pwd = $_POST['confirm_password'] ?? '';
            if (empty($pwd)) {
                $errors[] = 'Password confirmation is required to create a reservation.';
            } else {
                // verify current employee password (manager/operator)
                $mgrId = intval($_SESSION['employee_id'] ?? 0);
                $ep = $pdo->prepare('SELECT password FROM Employees WHERE employee_ID = ? LIMIT 1'); $ep->execute([$mgrId]); $mgrHash = $ep->fetchColumn() ?: '';
                if (!password_verify($pwd, $mgrHash)) {
                    $errors[] = 'Invalid password confirmation.';
                } else {
                    // load waitlist row
                    $s = $pdo->prepare('SELECT * FROM waitlist WHERE waitlist_ID = ? LIMIT 1'); $s->execute([$wid]); $w = $s->fetch(PDO::FETCH_ASSOC) ?: null;
                    if (empty($w)) { $errors[] = 'Waitlist entry not found.'; }
                    else {
                        $size = $w['preferred_slip_size'] ?? null; $start = $w['preferred_start_date'] ?? null; $end = $w['preferred_end_date'] ?? null;
                        if (empty($size) || empty($start) || empty($end)) { $errors[] = 'Waitlist entry missing required dates or slip size.'; }
                        else {
                            // find an available slip
                            $av = $pdo->prepare("SELECT s.slip_ID FROM slips s WHERE s.slip_size = ? AND s.slip_ID NOT IN (SELECT r.slip_ID FROM reservations r WHERE r.reservation_status IN ('confirmed','checked_in') AND NOT (r.end_date < ? OR r.start_date > ?)) ORDER BY s.location_code ASC LIMIT 1");
                            $av->execute([$size, $start, $end]); $srow = $av->fetch(PDO::FETCH_ASSOC) ?: null;
                            if (empty($srow) || empty($srow['slip_ID'])) { $errors[] = 'No available slip found for the requested dates.'; }
                            else {
                                try {
                                    $ins = $pdo->prepare('INSERT INTO reservations (user_ID, boat_ID, slip_ID, start_date, end_date, reservation_status, date_created) VALUES (?,?,?,?,?,"confirmed",NOW())');
                                    $ins->execute([$w['user_ID'],$w['boat_ID'],$srow['slip_ID'],$start,$end]);
                                    // remove the waitlist entry
                                    $d = $pdo->prepare('DELETE FROM waitlist WHERE waitlist_ID = ?'); $d->execute([$wid]);
                                    $_SESSION['statusMsg'] = 'Reservation created from waitlist.';
                                    if (!headers_sent()) { header('Location: ' . basename(__FILE__)); exit; }
                                    else { echo '<script>window.location = "' . basename(__FILE__) . '";</script>'; exit; }
                                } catch (Exception $ex) { $errors[] = 'Error creating reservation: ' . $ex->getMessage(); }
                            }
                        }
                    }
                }
...
<head>
    ...
    /* add extra vertical space below the Search header */
    .search-panel > h3 { margin-bottom: 15px; }
    /* compact metadata used in waitlist rows */
    .compact-meta{ display:flex; gap:18px; margin-top:6px; color:#334155; font-size:0.95rem }
    .compact-meta strong{ font-weight:700; margin-right:6px }
    /* narrow ID column for compact actions (Edit / Remove) */
    .col-id{ width:88px; max-width:88px; text-align:left; font-size:0.95rem }
    /* Quick tabs for task-driven searches */
    .search-panel .quick-tabs{ display:flex; gap:10px; margin-bottom:10px; align-items:center }
    .search-panel .quick-tabs .btn{ padding:10px 14px; border-radius:10px; font-weight:600; font-size:0.95rem; background:transparent; border:1px solid rgba(31,47,69,0.08); color:#0f1724; height:44px; min-width:140px; display:inline-flex; align-items:center; justify-content:center }
    ...
    #task-waitlist-delete{ border-color: rgba(212,175,55,0.12); color:var(--gold) }
    ...
    /* Today's Reservations quick link: ocean blue with white text */
    #task-today-res{ background:var(--ocean,#3F87A6); color:#fff; border-color:transparent }
    #task-today-res[aria-pressed="true"]{ box-shadow:0 8px 22px rgba(63,135,166,0.14); border-color:transparent }
    /* Completions quick tab shown as ocean-outline by default, ocean background when active */
    #task-completions{ border-color: rgba(63,135,166,0.12); color:var(--ocean) }
    #task-completions[aria-pressed="true"]{ background:var(--ocean,#3F87A6); color:#fff; border-color:transparent; box-shadow:0 8px 22px rgba(63,135,166,0.14) }
    /* (no changes to search-panel positioning) */
    /* #task-checkins removed */
    /* removed old gold active style for completions */
    #task-waitlist-delete[aria-pressed="true"]{ background:var(--gold,#D4AF37); color:#071018; border-color:transparent; box-shadow:0 8px 22px rgba(212,175,55,0.12) }
    ...
    /* Invert on hover: filled ocean background with white text */
    .btn-outline.btn-ocean:hover{ background:var(--ocean,#3F87A6); color:#fff; border-color:transparent; box-shadow:0 8px 20px rgba(63,135,166,0.12); }
    ...
    .btn-outline.btn-coral{ border-color:var(--coral,#FF7F50); color:var(--coral,#FF7F50); background:transparent }
    .btn-outline.btn-coral:hover{ background:var(--coral,#FF7F50); color:#fff; box-shadow:0 8px 20px rgba(255,127,80,0.12); }
...
<body>
    ...
    <div id="payment-modal" style="display:none;position:fixed;inset:0;background:rgba(11,17,32,0.5);align-items:center;justify-content:center;z-index:100003">
        <div style="background:#fff;padding:16px;border-radius:12px;max-width:520px;width:calc(100% - 40px);box-shadow:0 12px 36px rgba(11,17,32,0.3);">
            ...
            <?php if (false && strtolower(trim($employeePosition)) === 'manager'): ?>
                <!-- Add-employee button intentionally removed from payment modal to avoid duplicate controls -->
            <?php endif; ?>
        </div>
    </div>

        <?php /* flash/status message shown next to Add button below to avoid overlapping the search card */ ?>
        ...
        <?php if (!empty($_SESSION['employee_search_debug'])): ?>
...
Params: <?php echo e(json_encode($dbg['params'] ?? [])); ?>
POST: <?php echo e(json_encode($dbg['post'] ?? [])); ?></div>
            </div>
        <?php endif; ?>

        <?php
            // Only show the inline Edit User/Boat forms when explicitly requested via the edit actions
            // Use `$_REQUEST` so GET query parameters (used by client navigation) also trigger the inline render.
            $showEditUserForm = (isset($action) && $action === 'edit_user') || (!empty($_REQUEST['action']) && $_REQUEST['action'] === 'edit_user');
            $showEditBoatForm = (isset($action) && $action === 'edit_boat') || (!empty($_REQUEST['action']) && $_REQUEST['action'] === 'edit_boat');
        ?>
        ...
        <section class="search-panel hero-overlap">
            ...
            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
                <div class="quick-tabs" role="tablist" aria-label="Quick searches">
                    ...
                    <button type="button" id="task-completions" class="btn btn-outline" role="tab" aria-pressed="false">Checking Out (24h)</button>
            ...
            <form id="search-form" method="post" style="display:flex;gap:12px;flex-wrap:wrap;align-items:end">
                ...
                <div id="basic-search-row" style="display:flex;gap:12px;flex-wrap:wrap;align-items:end;width:100%">
                    <label class="field" style="min-width:140px"><span>Date</span><input type="date" name="date" value="<?php echo e((isset($_POST['action']) && $_POST['action']==='search') ? ($_POST['date'] ?? '') : ''); ?>"></label>
                    <label class="field" style="min-width:160px"><span>Scope</span>
                        <select name="scope">
                            ...
                            <?php if (strtolower(trim($employeePosition)) === 'manager'): ?>
                                <option value="employees" <?php if(($selectedScope ?? '')==='employees') echo 'selected'; ?>>Employees</option>
                            <?php endif; ?>
                    ...
                    <label class="field" style="min-width:140px"><span>Slip Size</span>
                        <select name="filter_slip_size">
                            ...
                            <?php foreach($availableSlipSizes as $ssx): ?>
                                <option value="<?php echo e($ssx); ?>" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['filter_slip_size'] ?? '')===$ssx)) echo 'selected'; ?>><?php echo e($ssx); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </label>
                    <label class="field" style="min-width:160px"><span>Status</span>
                        <select name="filter_status">
                            ...
                            <?php foreach ($availableStatuses as $sopt) : ?>
                                <option value="<?php echo e($sopt); ?>" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['filter_status'] ?? '')===$sopt)) echo 'selected'; ?>><?php echo e(ucfirst($sopt)); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </label>
                    <label class="field" style="min-width:220px"><span>Email</span><input type="email" name="email" value="<?php echo e((isset($_POST['action']) && $_POST['action']==='search') ? ($_POST['email'] ?? '') : ''); ?>"></label>
                </div>
                ...
                <div class="search-options-panel" id="search-options-panel" aria-hidden="true" style="display:none;width:100%">
                            ...
                            <label style="display:flex;flex-direction:column;margin-bottom:6px"><span>Group By</span>
                                <select name="group_by">
                                    <option value="">None</option>
                                            <option value="date" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['group_by'] ?? '')==='date')) echo 'selected'; ?>>Date</option>
                                            <option value="status" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['group_by'] ?? '')==='status')) echo 'selected'; ?>>Status</option>
                                            <option value="dock" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['group_by'] ?? '')==='dock')) echo 'selected'; ?>>Dock</option>
                                </select>
                            </label>
                            <!-- Status and Slip Size are already present in the basic row; avoid duplicates here -->
                            <label style="display:flex;flex-direction:column;margin-bottom:6px"><span>Owner</span><input type="text" name="filter_owner" value="<?php echo e((isset($_POST['action']) && $_POST['action']==='search') ? ($_POST['filter_owner'] ?? '') : ''); ?>" placeholder="Name contains"></label>
                            <label style="display:flex;flex-direction:column;margin-bottom:6px"><span>Phone</span><input type="text" name="filter_phone" value="<?php echo e((isset($_POST['action']) && $_POST['action']==='search') ? ($_POST['filter_phone'] ?? '') : ''); ?>" placeholder="Phone"></label>
                            <label style="display:flex;flex-direction:column;margin-bottom:6px"><span>Boat</span><input type="text" name="filter_boat" value="<?php echo e((isset($_POST['action']) && $_POST['action']==='search') ? ($_POST['filter_boat'] ?? '') : ''); ?>" placeholder="Boat name contains"></label>
                            <label style="display:flex;flex-direction:column;margin-bottom:6px"><span>Payment</span>
                                <select name="filter_payment">
                                    <option value="">Any</option>
                                    <option value="paid" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['filter_payment'] ?? '')==='paid')) echo 'selected'; ?>>Paid</option>
                                    <option value="unpaid" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['filter_payment'] ?? '')==='unpaid')) echo 'selected'; ?>>Unpaid</option>
                                </select>
                            </label>
                            <label style="display:flex;flex-direction:column;margin-bottom:6px"><span>Slip</span>
                                <select name="dock">
                                    ...
                                    <?php foreach($availableDocks as $dck): ?>
                                        <option value="<?php echo e($dck); ?>" <?php if((isset($_POST['action']) && $_POST['action']==='search' && ($_POST['dock'] ?? '')===$dck)) echo 'selected'; ?>><?php echo e($dck); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </label>
                        </div>
                    ...
                    <div style="display:flex;gap:8px;align-items:center;margin-bottom:-35px">
        ...
        <?php if (!empty($_SESSION['employee_id']) && strtolower(trim($employeePosition)) === 'manager'): ?>
            <div style="margin:-29px 0 24px 0;">
                <form method="post" style="display:inline-block">
                    <input type="hidden" name="action" value="add_employee">
                    <button class="btn btn-outline btn-ocean">Add New Employee</button>
                </form>
                <?php if (!empty($statusMsg) && stripos($statusMsg,'employee') !== false): ?>
                    <div style="display:inline-block;margin-left:12px;vertical-align:middle;padding:8px 12px;background:#e6ffef;border:1px solid #c6f0d6;border-radius:8px;color:#085a2a"><?php echo e($statusMsg); ?></div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        <script>
        (function(){
            ...
            var shouldOpen = <?php echo ((isset($_POST['action']) && $_POST['action']==='search') && (!empty($_POST['filter_owner'])||!empty($_POST['filter_boat'])||!empty($_POST['filter_phone'])||!empty($_POST['filter_payment'])||!empty($_POST['group_by'])||!empty($_POST['filter_slip_size']))) ? 'true' : 'false'; ?>;
        ...
        <?php if (!empty($showEditUserForm) && !empty($editUser)): ?>
            <section class="inline-edit" style="margin-top:6px;margin-left:6px;background:#fff;padding:16px;border-radius:10px;">
                ...
                <form method="post" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start">
                    ...
                    <?php endif; ?>
                    <div style="width:100%;margin-top:8px"><button type="submit" class="btn btn-ocean">Save User</button> <a href="employee_dashboard.php" class="btn btn-outline btn-cancel">Cancel</a></div>
                </form>
            </section>
        <?php endif; ?>
        ...
        <?php if (!empty($showEditBoatForm) && !empty($editBoat)): ?>
            <section class="inline-edit" style="margin-top:16px;background:#fff;padding:16px;border-radius:10px;">
                ...
                <form method="post" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start">
                    ...
                    <div style="width:100%;margin-top:8px"><button type="submit" class="btn btn-ocean">Save Boat</button> <a href="employee_dashboard.php" class="btn btn-outline btn-cancel">Cancel</a></div>
                </form>
            </section>
        <?php endif; ?>
        <?php
            // Show Add Employee form when manager requested it, or re-show after create attempt with errors
            $showAddEmployeeForm = (!empty($_REQUEST['action']) && ($_REQUEST['action'] === 'add_employee' || ($_REQUEST['action'] === 'create_employee' && !empty($errors))));
        ?>
        <?php if (!empty($showAddEmployeeForm)): ?>
            <?php
                $phoneField = null;
                if (!empty($employeePhoneCol)) $phoneField = $employeePhoneCol;
            ?>
            <section class="inline-edit" style="margin-top:16px;background:#fff;padding:16px;border-radius:10px;">
                <h3>Add Employee</h3>
                <form method="post" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start">
                    <input type="hidden" name="action" value="create_employee">
                    <label style="display:flex;flex-direction:column"><span>First Name</span><input type="text" name="first_name" value="<?php echo e($_POST['first_name'] ?? ''); ?>"></label>
                    <label style="display:flex;flex-direction:column"><span>Last Name</span><input type="text" name="last_name" value="<?php echo e($_POST['last_name'] ?? ''); ?>"></label>
                    <label style="display:flex;flex-direction:column"><span>Email</span><input type="email" name="email" value="<?php echo e($_POST['email'] ?? ''); ?>"></label>
                    <?php if ($phoneField): ?>
                        <label style="display:flex;flex-direction:column"><span>Phone</span><input type="tel" name="<?php echo e($phoneField); ?>" value="<?php echo e($_POST[$phoneField] ?? ''); ?>"></label>
                    <?php endif; ?>
                    <label style="display:flex;flex-direction:column"><span>Position</span><input type="text" name="position" value="<?php echo e($_POST['position'] ?? ''); ?>"></label>
                    <label style="display:flex;flex-direction:column"><span>New Employee Password</span><input type="password" name="new_password" required></label>
                    <label style="display:flex;flex-direction:column"><span>Manager Password (confirm)</span><input type="password" name="confirm_password" required></label>
                    <div style="width:100%;margin-top:8px"><button type="submit" class="btn btn-ocean">Create Employee</button> <a href="employee_dashboard.php" class="btn btn-outline btn-cancel">Cancel</a></div>
                </form>
            </section>
        <?php endif; ?>

        <?php if (!empty($editEmployee)): ?>
            <section class="inline-edit" style="margin-top:16px;background:#fff;padding:16px;border-radius:10px;">
                <h3>Edit Employee #<?php echo e($editEmployee['employee_ID'] ?? ($editEmployee['id'] ?? '')); ?></h3>
                <form method="post" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start">
                    <input type="hidden" name="action" value="save_employee">
                    <input type="hidden" name="employee_id" value="<?php echo e($editEmployee['employee_ID'] ?? ($editEmployee['id'] ?? '')); ?>">
                    <label style="display:flex;flex-direction:column"><span>First Name</span><input type="text" name="first_name" value="<?php echo e($editEmployee['first_name'] ?? ''); ?>"></label>
                    <label style="display:flex;flex-direction:column"><span>Last Name</span><input type="text" name="last_name" value="<?php echo e($editEmployee['last_name'] ?? ''); ?>"></label>
                    <label style="display:flex;flex-direction:column"><span>Email</span><input type="email" name="email" value="<?php echo e($editEmployee['email'] ?? ''); ?>"></label>
                    <?php 
                        $phoneField = null;
                        if (!empty($employeePhoneCol)) $phoneField = $employeePhoneCol;
                        elseif (is_array($editEmployee) && array_key_exists('phone', $editEmployee)) $phoneField = 'phone';
                    ?>
                    <?php if ($phoneField): ?>
                        <label style="display:flex;flex-direction:column"><span>Phone</span><input type="tel" name="<?php echo e($phoneField); ?>" value="<?php echo e($editEmployee[$phoneField] ?? ''); ?>"></label>
                    <?php endif; ?>
                    <label style="display:flex;flex-direction:column"><span>Position</span><input type="text" name="position" value="<?php echo e($editEmployee['position'] ?? ''); ?>"></label>
                    <label style="display:flex;flex-direction:column"><span>Employee Password (confirm)</span><input type="password" name="confirm_password" required></label>
                    <div style="width:100%;margin-top:8px"><button type="submit" class="btn btn-ocean">Save Employee</button> <a href="employee_dashboard.php" class="btn btn-outline btn-cancel">Cancel</a></div>
                </form>
            </section>
        <?php endif; ?>
            <script>
    ...
    (function(){
                ...
                try {
                    ...
                    try {
                        if(ok && typeof ok.textContent !== 'undefined'){
                            if(act === 'complete') { ok.textContent = 'Complete'; ok.className = 'btn btn-ocean'; }
        ...
        <?php if (!empty($editReservation)): ?>
            <section style="margin-top:16px;background:#fff;padding:16px;border-radius:10px;">
                ...
                <form method="post">
                    ...
                    <?php endif; ?>
                    <div style="margin-top:8px;"><button type="submit" class="btn btn-ocean">Save</button> <a href="employee_dashboard.php" class="btn btn-outline btn-cancel">Cancel</a></div>
                </form>
        ...
        <?php
            ...
            if (empty($group_by) && !empty($searchResults)) {
                ...
                // treat task-driven result types (tasks_*) as reservations for rendering
                if (is_string($scopeKey) && strpos($scopeKey, 'tasks_') === 0) { $scopeKey = 'reservations'; }
        ...
        <?php if (!empty($searchResults)): ?>
            <section class="results panel" style="margin-top:18px;">
                ...
                <?php if (isset($resultType) && $resultType === 'tasks_checkins'): ?>
                    <?php
                        $diag_total = count($searchResults);
                        $diag_first_status = $searchResults[0]['reservation_status'] ?? 'N/A';
                        // compute statusGroups as in main rendering to test
                        $diag_statusGroups = [];
                        foreach ($searchResults as $rr) {
                            $st = strtolower(trim((string)($rr['reservation_status'] ?? 'unknown')));
                            if ($st === '') $st = 'unknown';
                            $diag_statusGroups[$st][] = $rr;
                        }
                        $diag_ordered = [];
                        if (isset($diag_statusGroups['checked_in'])) $diag_ordered[] = 'checked_in';
                        foreach (array_keys($diag_statusGroups) as $k) { if ($k === 'checked_in') continue; $diag_ordered[] = $k; }
                    ?>
                    <div style="background:#eef6ff;padding:8px;border-radius:6px;margin:8px 0;font-family:monospace;color:#063">
                        Diag: total=<?php echo e($diag_total); ?>; first_status=<?php echo e($diag_first_status); ?>; ordered_statuses=<?php echo e(json_encode($diag_ordered)); ?>
                    </div>
                    <div style="background:#f7fff7;padding:8px;border-radius:6px;margin:8px 0;font-family:monospace;color:#055">First row: <?php echo e(json_encode($searchResults[0] ?? [])); ?></div>
                <?php endif; ?>
            <?php if ($scopeKey === 'reservations'): ?>
                    <?php
                ...
                <?php elseif ($scopeKey === 'waitlist'): ?>
                            ...
                            <table>
                                ...
                                <?php foreach($items as $w): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php echo e($w['boat_name'] ?? ''); ?><?php if(!empty($w['boat_length'])): ?> <small>(<?php echo e($w['boat_length']); ?> ft)</small><?php endif; ?>
                                            <div class="compact-meta">
                                                <?php
                                                    $ownerDisplay = trim((($w['first_name'] ?? '') . ' ' . ($w['last_name'] ?? '')) ?: ($w['email'] ?? ''));
                                                    $rowName = trim((($w['first_name'] ?? '') . ' ' . ($w['last_name'] ?? '')));
                                                ?>
                                                <?php if(!empty($ownerDisplay) && $ownerDisplay !== $rowName): ?>
                                                    <div><strong>Owner:</strong> <?php echo e($ownerDisplay); ?></div>
                                                <?php endif; ?>
                                                <div><strong>Email:</strong> <?php echo e($w['email'] ?? ''); ?></div>
                                                <div><strong>Phone:</strong> <?php echo e($w['user_phone'] ?? ''); ?></div>
                                            </div>
                                        </td>
                                    ...
                                    <!-- details moved inline into Boat cell -->
                                <?php endforeach; ?>
                                </tbody>
                            </table>
                        <?php endforeach; ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                            ...
                                            $end = $w['preferred_end_date'] ?? null;
                                            $available = 0;
                                            if (!empty($size) && !empty($start) && !empty($end)) {
                                                $sc = $pdo->prepare('SELECT COUNT(*) FROM slips WHERE slip_size = ?'); $sc->execute([$size]); $totalSlips = intval($sc->fetchColumn() ?? 0);
                                                $rc = $pdo->prepare("SELECT COUNT(DISTINCT r.slip_ID) FROM reservations r JOIN slips sl ON r.slip_ID = sl.slip_ID WHERE sl.slip_size = ? AND r.reservation_status IN ('confirmed','checked_in') AND NOT (r.end_date < ? OR r.start_date > ?)");
                                                $rc->execute([$size, $start, $end]); $reserved = intval($rc->fetchColumn() ?? 0); $available = max(0, $totalSlips - $reserved);
                                            }
                                        ?>
                                        <form method="post" style="display:inline-block;"><input type="hidden" name="action" value="edit_waitlist"><input type="hidden" name="waitlist_id" value="<?php echo e($w['waitlist_ID']); ?>"><button class="btn btn-outline btn-ocean">Edit</button></form>
                                        <?php if(!empty($available) && $available > 0): ?>
                                            <form method="post" style="display:inline-block;margin-left:8px" onsubmit="return confirmAddReservation(this);">
                                                <input type="hidden" name="action" value="add_reservation_from_waitlist">
                                                <input type="hidden" name="waitlist_id" value="<?php echo e($w['waitlist_ID']); ?>">
                                                <input type="hidden" name="confirm_password" value="">
                                                <button class="btn btn-outline btn-green">Add to Reservations</button>
                                            </form>
                                        <?php else: ?>
                                            <span style="display:inline-block;margin-left:8px;color:#64748b;font-size:0.9rem">No slips</span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                    ...
                    <?php endif; ?>
                <?php elseif ($scopeKey === 'employees'): ?>
                    <table>
                        <thead><tr><th class="col-id">Employee ID</th><th>Name</th><th>Email</th><th>Phone</th><th>Position</th><th>Date Created</th><th></th></tr></thead>
                        <tbody>
                        <?php foreach($searchResults as $e): ?>
                            <tr>
                                <td><?php echo e($e['employee_ID']); ?></td>
                                <td><?php echo e(trim(($e['first_name'] ?? '') . ' ' . ($e['last_name'] ?? ''))); ?></td>
                                <td><?php echo e($e['email'] ?? ''); ?></td>
                                <td>
                                    <?php
                                        $phone = '';
                                        if (!empty($employeePhoneCol) && isset($e[$employeePhoneCol])) {
                                            $phone = $e[$employeePhoneCol];
                                        } else {
                                            $phone = $e['phone'] ?? $e['user_phone'] ?? $e['phone_number'] ?? $e['home_phone'] ?? '';
                                        }
                                    ?>
                                    <?php echo e($phone); ?>
                                </td>
                                <td><?php echo e($e['position'] ?? ''); ?></td>
                                <td><?php echo e(formatDateShort($e['date_created'] ?? '')); ?></td>
                                <td>
                                    <form method="post" style="display:inline-block">
                                        <input type="hidden" name="action" value="edit_employee">
                                        <input type="hidden" name="employee_id" value="<?php echo e($e['employee_ID']); ?>">
                                        <button class="btn btn-outline btn-ocean">Edit</button>
                                    </form>
                                    <?php if (strtolower(trim($employeePosition)) === 'manager'): ?>
                                        <form method="post" style="display:inline-block;margin-left:8px" onsubmit="return confirmRemoveEmployee(this);">
                                            <input type="hidden" name="action" value="remove_employee">
                                            <input type="hidden" name="employee_id" value="<?php echo e($e['employee_ID']); ?>">
                                            <input type="hidden" name="confirm_password" value="">
                                            <button class="btn btn-outline btn-coral">Remove</button>
                                        </form>
                                    <?php endif; ?>
                                </td>
                            </tr>
    ...
    (function(){
        ...
        function clearSearchFilters(frm){
            var names = ['date','filter_owner','filter_phone','filter_boat','filter_payment','dock'];
            names.forEach(function(n){ var el = frm.querySelector('[name="'+n+'"]'); if(el){ if(el.tagName==='SELECT') el.selectedIndex = 0; else el.value=''; }});
        }
        // Today's Check-ins removed (no handler)
        if(taskCompletions && form && taskField){ taskCompletions.addEventListener('click', function(ev){ ev.preventDefault(); activateQuickTab(taskCompletions); taskField.value = 'completions_today'; var scope=form.querySelector('select[name="scope"]'); if(scope) scope.value='reservations'; clearSearchFilters(form); setTimeout(function(){ form.submit(); }, 50); }); }
        if(taskWl && form && taskField){ taskWl.addEventListener('click', function(ev){ ev.preventDefault(); activateQuickTab(taskWl); taskField.value = 'waitlist_deletions'; var scope=form.querySelector('select[name="scope"]'); if(scope) scope.value='waitlist'; clearSearchFilters(form); setTimeout(function(){ form.submit(); }, 50); }); }
        if(taskPayment24 && form && taskField){ taskPayment24.addEventListener('click', function(ev){ ev.preventDefault(); activateQuickTab(taskPayment24); taskField.value = 'payments_due_24h'; var scope=form.querySelector('select[name="scope"]'); if(scope) scope.value='reservations'; clearSearchFilters(form); setTimeout(function(){ form.submit(); }, 50); }); }
        ...
        // Confirm remove employee handler: prompt for manager password then confirm
        window.confirmRemoveEmployee = function(form){
            var pwd = prompt('Enter your manager password to confirm removal:');
            if (pwd === null) return false;
            if (!pwd) { alert('Password is required to remove an employee.'); return false; }
            var inp = form.querySelector('input[name="confirm_password"]');
            if (!inp) { inp = document.createElement('input'); inp.type='hidden'; inp.name='confirm_password'; form.appendChild(inp); }
            inp.value = pwd;
            return confirm('Are you sure you want to remove this employee? This action cannot be undone.');
        };
        // Confirm add reservation from waitlist: ask for password then submit
        window.confirmAddReservation = function(form){
            var pwd = prompt('Enter your password to confirm creating this reservation:');
            if (pwd === null) return false;
            if (!pwd) { alert('Password is required to confirm.'); return false; }
            var inp = form.querySelector('input[name="confirm_password"]');
            if (!inp) { inp = document.createElement('input'); inp.type='hidden'; inp.name='confirm_password'; form.appendChild(inp); }
            inp.value = pwd;
            return confirm('Create a confirmed reservation for this waitlist entry?');
        };
            ...
            .then(function(data){
                ...
                keys.forEach(function(len){
                    ...
                    (data[len] || []).forEach(function(b){
                        ...
                        title.textContent = (b.boat_name || 'Unnamed');
                        li.appendChild(title);
                        // Length (plain) shown below the name
                        if (typeof b.boat_length !== 'undefined' && b.boat_length) {
                            var lengthLine = document.createElement('div');
                            lengthLine.style.fontSize = '0.9rem';
                            lengthLine.style.color = '#555';
                            lengthLine.textContent = (b.boat_length + ' ft');
                            li.appendChild(lengthLine);
                        }
                        list.appendChild(li);
                    });
    ...
    (function(){
        function submitEdit(action, id){
            ...
            // Navigate using query params so server can render the inline edit in-place.
            try {
                var base = window.location.pathname || window.location.href.split('?')[0];
                var params = new URLSearchParams();
                params.set('action', action);
                if(action === 'edit_user') params.set('user_id', id); else params.set('boat_id', id);
                var currentResInput = document.querySelector('input[name="reservation_id"]');
                if (currentResInput && currentResInput.value) params.set('reservation_id', currentResInput.value);
                // preserve any fragment
                var frag = window.location.hash || '';
                window.location.href = base + '?' + params.toString() + frag;
            } catch(e) { console.error('submitEdit navigation failed', e); alert('Unable to open edit form'); }
        }
        document.addEventListener('click', function(e){
            try{
    ...
    <script>
    // Ensure footer sits at bottom of viewport when page content is short (non-invasive)
    (function(){
        function adjustFooter(){
            ...
            f.style.bottom = '';
            f.style.width = '';
            // If page content height is less than or equal to viewport, pin footer to bottom
            if (document.body.scrollHeight <= window.innerHeight) {
                var h = f.offsetHeight || 40;
                f.style.position = 'absolute';
                f.style.left = '0';
                f.style.right = '0';
                f.style.bottom = '0';
                f.style.width = '100%';
                // add body padding so footer doesn't overlap content
