...
-- create database first, then users/grants
CREATE DATABASE IF NOT EXISTS moffat_bay DEFAULT CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci;
USE moffat_bay;

-- ensure consistent username and MariaDB-friendly auth
DROP USER IF EXISTS 'marina_user'@'localhost';
CREATE USER 'marina_user'@'localhost' IDENTIFIED BY 'moffatbaymarina';
...
-- EMPLOYEE PAYMENTS
CREATE TABLE IF NOT EXISTS employees_payments (
...
reservation_status ENUM('confirmed','canceled','checked_in','completed') NOT NULL DEFAULT 'confirmed',
...
-- TRIGGER (DELIMITER required for routines/triggers; client-side)
...
-- Employees
INSERT INTO employees (employee_ID,email,password,first_name,last_name,phone,position,date_created) VALUES
(1,'eve.tucker@example.com','<hashed_pw>','Eve','Tucker','555-324-5436','Manager','2026-01-01 08:00:00'),
(2,'charlie.roberts@example.com','<hashed_pw>','Charlie','Roberts','555-225-6473','Dockhand','2026-01-02 08:30:00'),
(3,'sam.lewis@example.com','<hashed_pw>','Sam','Lewis','555-235-5473','Supervisor','2026-01-03 09:00:00');

-- Employee Payments
INSERT INTO employee_payments (payment_ID,reservation_ID,amount,method,card_last4,employee_ID,date_created) VALUES
(1,1,'700.00','card','7586',1,'2026-02-24 18:54:00'),
(2,2,'230.00,'cash','NULL',1,2026-02-25 17:39:48'),
(3,3,'490.00,'cash', 'NULL',1, 2026-02-25 18:25:24');
