...
<body>
  ...
  <section class="page">
    ...
    <?php if (!empty($activeReservations) || !empty($pastReservations)): ?>
      <!-- RESERVATIONS_TEMPLATE_UPDATED: 2026-02-10 -->
      <h1 style="margin-top:18px">Your Reservations</h1>
      <div class="account-card" style="margin-top:18px">
        <?php if (!empty($activeReservations)): ?>
          <h3 class="section-title">Active / Upcoming</h3>
          <ul style="list-style:none;padding:0;margin:0">
            <?php foreach ($activeReservations as $r): ?>
              <?php $status = strtolower($r['reservation_status'] ?? $r['status'] ?? $r['state'] ?? ''); ?>
              <li class="reservation-item">
                <div class="reservation-main">
                  <div class="reservation-title"><strong><?=htmlspecialchars($r['title'] ?? $r['reservation_name'] ?? 'Reservation')?></strong></div>
                  <div class="reservation-meta"><span class="reservation-label">Dates:</span>
                    <?php
                      $start = $r['start_date'] ?? $r['date'] ?? $r['reservation_date'] ?? $r['created_at'] ?? null;
                      $end = $r['end_date'] ?? null;
                  ...
                  <?php if (!empty($r['boat_name']) || !empty($r['boat_length'])): ?>
                    <div class="reservation-meta"><span class="reservation-label">Boat:</span><?=htmlspecialchars($r['boat_name'] ?? '')?><?=(!empty($r['boat_length']) ? ' — ' . htmlspecialchars($r['boat_length']) . ' ft' : '')?></div>
                  <?php endif; ?>
                  <div class="reservation-meta"><span class="reservation-label">Status:</span><?=htmlspecialchars($r['reservation_status'] ?? $r['status'] ?? $r['state'] ?? 'Unknown')?></div>
                  <div class="reservation-meta"><span class="reservation-label">Slip:</span><?=htmlspecialchars($r['slip_location'] ?? $r['location'] ?? $r['slip_ID'] ?? $r['slip_number'] ?? $r['slot'] ?? '')?></div>
                </div>
                <div class="reservation-right">
                  <?php if ($status === 'confirmed'): ?>
                    <div style="display:flex;gap:8px;align-items:center">
                      <button class="btn save reservation-edit-btn" type="button" data-reservation-id="<?=intval($r['reservation_ID'] ?? $r['reservation_id'] ?? $r['id'] ?? 0)?>">Edit</button>
                      <button class="btn reservation-cancel-btn" type="button" data-reservation-id="<?=intval($r['reservation_ID'] ?? $r['reservation_id'] ?? $r['id'] ?? 0)?>">Cancel</button>
                    </div>
                  <?php endif; ?>
                </div>
                <div class="reservation-edit-form" style="display:none;margin-top:10px;width:100%">
                  <form method="post" style="display:flex;flex-direction:column;gap:8px">
                    <input type="hidden" name="reservation_action" value="edit">
                    <input type="hidden" name="reservation_id" value="<?=intval($r['reservation_ID'] ?? $r['reservation_id'] ?? $r['id'] ?? 0)?>">
                    <label style="font-weight:700">Reservation Name</label>
                    <input type="text" name="reservation_name" value="<?=htmlspecialchars($r['title'] ?? $r['reservation_name'] ?? '')?>">
                    <div style="display:flex;gap:8px">
                      <div style="flex:1">
                        <label style="font-weight:700">Start Date</label>
                        <input type="date" name="start_date" value="<?=htmlspecialchars(isset($r['start_date']) ? date('Y-m-d', strtotime($r['start_date'])) : '')?>">
                      </div>
                      <div style="flex:1">
                        <label style="font-weight:700">End Date</label>
                        <input type="date" name="end_date" value="<?=htmlspecialchars(isset($r['end_date']) ? date('Y-m-d', strtotime($r['end_date'])) : '')?>">
                      </div>
                    </div>
                    <div style="display:flex;gap:8px;align-items:center">
                      <div style="flex:1">
                        <label style="font-weight:700">Boat</label>
                        <select name="boat_id">
                          <option value="">(No change)</option>
                          <?php foreach (($boats ?? []) as $bb): ?>
                            <option value="<?=intval($bb['boat_id'])?>" <?=(!empty($r['boat_ID']) && intval($r['boat_ID'])===intval($bb['boat_id']) ? 'selected' : '')?>><?=htmlspecialchars($bb['name'] ?? '')?></option>
                          <?php endforeach; ?>
                        </select>
                      </div>
                      <div style="flex:1">
                        <label style="font-weight:700">Slip (code or id)</label>
                        <input type="text" name="slip_location" value="<?=htmlspecialchars($r['slip_location'] ?? $r['location'] ?? $r['slip_ID'] ?? '')?>">
                      </div>
                    </div>
                    <label style="font-weight:700">Confirm Current Password</label>
                    <input type="password" name="current_password" required>
                    <div style="display:flex;gap:8px;justify-content:flex-end">
                      <button class="btn save" type="submit">Save</button>
                      <button type="button" class="btn cancel reservation-edit-cancel">Cancel</button>
                    </div>
                  </form>
                </div>
              </li>
        ...
        <?php if (!empty($pastReservations)): ?>
          <h3 class="section-title" style="margin-top:12px">Past Reservations</h3>
          <ul style="list-style:none;padding:0;margin:0">
