...
$passwordError = false;
// Detect employee sessions early so POST handlers can update Employees rows
$isEmployee = !empty($_SESSION['is_employee']);
  ...
  if (!in_array($boat_action, ['add','edit','delete']) && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    try {
      ...
      // load current account row to verify password and detect columns
      $current = false;
      $pkCol = null;
      // If this is an employee session, prefer the Employees table
      if (!empty($isEmployee)) {
        try {
          $empId = $_SESSION['employee_id'] ?? null;
          if ($empId) {
            $stmt = $pdo->prepare('SELECT * FROM Employees WHERE employee_ID = :id LIMIT 1');
            $stmt->execute([':id' => $empId]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row) { $current = $row; $pkCol = array_key_exists('employee_ID',$row) ? 'employee_ID' : (array_key_exists('employee_id',$row) ? 'employee_id' : (array_key_exists('id',$row) ? 'id' : null)); }
          }
          if (!$current && !empty($userEmail)) {
            $stmt = $pdo->prepare('SELECT * FROM Employees WHERE email = :email LIMIT 1');
            $stmt->execute([':email' => $userEmail]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row) { $current = $row; foreach (['employee_ID','employee_id','id'] as $c) { if (array_key_exists($c,$row)) { $pkCol = $c; break; } } }
          }
        } catch (Exception $e) {
      ...
      // If not found in Employees (or not an employee), fall back to users table
      if (!$current) {
        if ($userId) {
          $idCandidates = ['id', 'user_id', 'uid'];
          foreach ($idCandidates as $c) {
      ...
      // Choose target table: update Employees when current row came from Employees, otherwise users
      $targetTable = (array_key_exists('employee_ID', $current) || array_key_exists('employee_id', $current)) ? 'Employees' : 'users';
      $sql = 'UPDATE ' . $targetTable . ' SET ' . implode(', ', $fields) . ' WHERE ' . $where;
...
// If logged in as an employee (set by the login handler), attempt to load employee details
$isEmployee = !empty($_SESSION['is_employee']);
$employee = null;
if ($isEmployee && $pdoExists) {
  try {
    $empId = $_SESSION['employee_id'] ?? null;
    if ($empId) {
      $es = $pdo->prepare('SELECT * FROM Employees WHERE employee_ID = :id LIMIT 1');
      $es->execute([':id' => $empId]);
      $employee = $es->fetch(PDO::FETCH_ASSOC);
    }
    // fallback: try by email/username in session
    if (!$employee && !empty($_SESSION['username'])) {
      $es = $pdo->prepare('SELECT * FROM Employees WHERE email = :e LIMIT 1');
      $es->execute([':e' => $_SESSION['username']]);
      $employee = $es->fetch(PDO::FETCH_ASSOC);
    }
    if ($employee) {
      // Normalize fields for template compatibility
      $employee['name'] = trim(($employee['first_name'] ?? '') . ' ' . ($employee['last_name'] ?? '')) ?: ($employee['email'] ?? '');
      $employee['email'] = $employee['email'] ?? '';
      $employee['phone'] = $employee['phone'] ?? '';
      $employee['position'] = $employee['position'] ?? '';
      $employee['date_created'] = $employee['date_created'] ?? $employee['created_at'] ?? '';
    }
  } catch (Exception $e) {
    // ignore employee load errors; leave $employee null
  }
}

// Choose which account data to display:
// - If logged in as an employee and an Employees row was loaded, show the employee details only.
// - Otherwise prefer the normal user row and fall back to an employee row if present.
if (!empty($isEmployee) && !empty($employee)) {
  $accountData = $employee;
} else {
  $accountData = $user ?: $employee;
}
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .btn.cancel{background:transparent;color:var(--coral);border:1px solid var(--coral)}
    ...
    .btn.cancel:hover{background:var(--coral);color:#fff;border-color:var(--coral)}
...
<body>
        ...
        <form id="accountForm" method="post" action="MyAccount.php">
          <div class="account-card-header">
            <div>
              ...
              <?php if (!empty($isEmployee)): ?>
                <div class="muted" style="margin-top:6px">Employee account — you can update your contact details and password here. Position cannot be changed.</div>
              <?php else: ?>
          ...
          <div class="info-row">
            <div class="info-body">
              ...
              <div class="info-value"><input type="text" id="name" name="name" value="<?=htmlspecialchars($accountData['name'] ?? '')?>" disabled></div>
            </div>
          </div>
          ...
          <div class="info-row">
            <div class="info-body">
              ...
              <div class="info-value"><input type="tel" id="phone" name="phone" value="<?=htmlspecialchars($accountData['phone'] ?? '')?>" disabled></div>
            </div>
          </div>
          ...
          <div class="info-row">
            <div class="info-body">
              ...
              <div class="info-value"><input type="email" id="email" name="email" value="<?=htmlspecialchars($accountData['email'] ?? '')?>" disabled></div>
            </div>
          </div>
          ...
          <div class="info-row">
            <div class="info-body">
              ...
              <div class="info-value" style="font-weight:600"><?=htmlspecialchars($accountData['position'] ?? '')?></div>
            </div>
          </div>
          <?php endif; ?>
  ...
  <section class="page">
    <div class="account-card" style="border-left:4px solid rgba(232,137,107,0.18);">
      ...
      <div style="margin-top:12px">
        <style>
          /* Coral-outline buttons: transparent with coral text/border, fill on hover/focus */
          /* Specific known controls */
          #showDeleteAccount,
          #changePwBtn,
          .boat-delete-btn,
          .btn.coral-outline,
          .coral-outline,
          /* General catch-alls: any .btn with 'cancel' or inline coral color/border */
          .btn.cancel,
          button[style*="var(--coral)"],
          a[style*="var(--coral)"],
          .btn[style*="var(--coral)"],
          ...
          /* Hover / focus: invert to filled coral with white text */
          #showDeleteAccount:hover,
          #changePwBtn:hover,
          .boat-delete-btn:hover,
          .btn.coral-outline:hover,
          .coral-outline:hover,
          .btn.cancel:hover,
          button[style*="var(--coral)"]:hover,
          a[style*="var(--coral)"]:hover,
          .btn[style*="var(--coral)"]:hover,
          button[style*="color:var(--coral)"]:hover,
          .btn[style*="color:var(--coral)"]:hover,
          #showDeleteAccount:focus,
          #changePwBtn:focus,
          .boat-delete-btn:focus,
          .btn.coral-outline:focus,
          .coral-outline:focus,
          .btn.cancel:focus,
          button[style*="var(--coral)"]:focus,
          a[style*="var(--coral)"]:focus,
          .btn[style*="var(--coral)"]:focus,
          button[style*="color:var(--coral)"]:focus,
          .btn[style*="color:var(--coral)"]:focus {
...
(function(){
  function adjustFooter(){
    ...
    var spacerId = 'footer-spacer-for-employee';
    var spacer = document.getElementById(spacerId);
    // If page shorter than viewport, pin footer to bottom and insert spacer so layout doesn't shift
    if (bodyH < docH) {
      if (!spacer) {
      ...
      // set spacer height to footer height to preserve document flow
  ...
  // call now in case DOM already ready
  adjustFooter();
})();
</script>
<?php endif; ?>
