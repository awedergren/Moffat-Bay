...
$passwordError = false;
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && $pdoExists) {
    // Prevent employee sessions from submitting account changes via POST on this page.
    // Employee accounts are read-only here; any POSTs should be ignored.
    if (!empty($isEmployee)) {
    ...
    <div class="account-card">
      <?php if ($accountData): ?>
        <form id="accountForm" method="post" action="MyAccount.php">
          <div class="account-card-header">
            <div>
              ...
              <?php if (!empty($isEmployee)): ?>
                <div class="muted" style="margin-top:6px">Employee account — details are read-only here.</div>
              <?php else: ?>
          ...
          <div class="actions">
            <?php if (!empty($isEmployee)): ?>
              <div style="margin-right:auto;color:#6b7280;font-weight:600">Position: <?=htmlspecialchars($accountData['position'] ?? '')?></div>
            <?php else: ?>
