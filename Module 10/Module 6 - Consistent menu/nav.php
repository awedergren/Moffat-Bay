...
<header class="topbar">
  <style>
    ...
    /* Employee dashboard coral-outline style (inverts on hover) */
    nav .nav-left a.employee-dashboard{border:1px solid var(--coral);color:var(--coral);padding:8px 12px;border-radius:8px;font-weight:700;background:transparent}
    nav .nav-left a.employee-dashboard:hover{background:var(--coral);color:#ffffff}
    ...
    <nav>
      <div class="nav-left">
        <?php if (!empty($_SESSION['is_employee'])): ?>
          <a href="employee_dashboard.php" class="employee-dashboard">Employee Dashboard</a>
          <?php if ($loggedIn): ?>
